...
This problem is a generalisation of [question 21](/leetcode/linked-lists/21). We know there is a way to solve that in $\mathcal{O}(n)$ time with constant space, so if we can reduce this problem down to the two sorted lists problem, we can solve this efficiently. The question is then how can we convert $k$ sorted lists into 2 sorted lists? This is where a divide and conquer strategy works wonders: if we can combine $\frac{k}{2}$ lists into 2 lists, then doing so results in 2 sorted lists - and with this logic we can divide all the way down until there are only 2 lists to combine, and then work our way back up to combine $k$ total lists.
...
From question 21, `mergeTwoLists` takes $\mathcal{O}(n)$ time, and every other non-recursive operation is constant time. Since we are always dividing the list lengths in half, there are going to be $\log_2(k)$ divisions and therefore $\log_2(k)$ calls to `mergeKLists`. Therefore there are $\log_2(k)$ calls each in $\mathcal{O}(n)$ time, so the total time complexity of the algorithm is in $\mathcal{O}(n\log k)$ time.

## Space Complexity

Each call takes constant space, but there are $\log_2(k)$ calls to `mergeKLists` - so the total space complexity is in $\mathcal{O}(\log k)$.