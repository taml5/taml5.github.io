...
A naive bottom-up implementation uses default values for $h_{n+1}$ and $h_{n+2}$.
...
However, notice that calculating $OPT(i)$ only requires the previous two values, so we don't have to memoise every value.
...
Let $n$ be the number of houses. Then there is one loop with constant time iterations and $n$ iterations, so the algorithm takes $\mathcal{O}(n)$ time.
