...
If we ever reach a day $(0, j)$ that results in a loss, then we stop checking $(0, k)$ for $k > j$, since according to the third insight $(j, k)$ will always outperform $(0, k)$ anyways. At this point, we always start to buy on $j$, and we check the subsequent trades from that point onwards.
- Why don't we need to check $(i_0, k)$ for some $0 < i_0< j$ and move straight to checking $(j, k)$? Since the algorithm has moved the selling day past $i_0$ to $k$, that means that $(0, i_0)$ has resulted in a profit—in other words, $p_{i_0} \geq p_0$. Thus, we have $p_k - p_{i_0} \leq p_k - p_0 < p_k - p_j$, so $(i_0, k)$ always underperforms compared to $(j, k)$.
