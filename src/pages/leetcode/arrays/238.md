...
- Let $n$ be an array of $l$ integers. For $i \in \{0, \dotsc, l - 1\}$, let $\mathcal{p_i} = n_0 \times \dotsb \times n_{i - 1}$ be the prefix of $i$, and $\mathcal{s_i} = n_{i + 1} \times \dotsb \times n_{l -1}$ be the suffix of $i$. Then the answer $a_i$ is given by the equation$$a_i = p_i \times s_i.$$In other words, the answer is given by the product of everything before multiplied by the product of everything afterwards.
-  $p_i$ is dependant on the value of $p_{i - 1}$,: $$p_i = (n_0 \times \dotsb \times n_{i - 2}) \times n_{i - 1} = p_{i - 1} \times n_{i - 1}$$ Moreover, $p_0 = 1$ since there is no prefix for $a_0$.
- $s_{i - 1}$ is dependant on the value of $s_i$: $$s_{i - 1} =  n_{i} \times (n_{i + 1} \times \dotsb \times n_{l - 1}) = n_{i} \times s_{i}$$Moreover, $s_{l - 1} = 1$  since there is no suffix for $a_{l - 1}$.
